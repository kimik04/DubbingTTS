...
async def _generate_character_tts(
    ...
    for conn_attempt in range(5):
        try:
            ...
            if conn_attempt == 4:
                log.error(f"  {character}: could not connect after 5 attempts ({e})")
                return paths
            wait = 15 * (conn_attempt + 1)
            log.warning(f"  {character}: connect failed ({e}), waiting {wait}s...")
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        ...
                        break
                    wait = 10 * (attempt + 1)
                    log.warning(f"  {character} seg_{seg.index}: failed ({e}), waiting {wait}s and reconnecting...")
                    try:
                        await ws.close()
                    except:
                        pass
                    ...
                    try:
                        ...
                        log.warning(f"  reconnect failed ({conn_err}), waiting 30s...")
